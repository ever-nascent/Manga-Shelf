...
const manga_url = 'https://m.manganelo.com/manga-ee88598';
const title = 'high score girl';
let chapter_to_start_at = 1;

const main = async () => {
	Util.validate_path('./panels');
	Util.validate_path('./checkpoints');
...
const fetch_chapter_links = async (site_url) => {
	const checkpoint_exist = Util.validate_file(`./checkpoints/${title}-checkpoint.json`);
	...
	if (!checkpoint_exist) {
		...
		Util.initMangaCheckpoint(title, list_of_chapter_links);
		return list_of_chapter_links;
	}

	console.log('Checkpoint detected for requested Manga.');
	const mangaCheckpointData = fs.readFileSync(`./checkpoints/${title}-checkpoint.json`, 'utf-8');
	const mangaCheckpointObj = JSON.parse(mangaCheckpointData);
	return mangaCheckpointObj.chapter_links;
}
...
const fetch_panel_links = async (chapter_links, chapter_to_start_at) => {
	...
	for (let i = chapter_to_start_at; i < chapter_links.length; i++) {
		console.log(`Fetching Panel Links for Chapter ( ${i + 1} / ${chapter_links.length} )`)
...
const download_panels = async(panel_links) => {
	for (let i = 0; i < panel_links.length; i++) {
		...
		console.log(`Starting Panel Downloads for Chapter ${chapter_to_start_at + (chapter_num + 1)}`)

		for (let j = 0; j < panel_links[i].length; j++) {
			const panel_link = panel_links[i][j];
			const extension = panel_link.slice(-3);

			Util.validate_path(`./panels/${title}-chapter-${chapter_to_start_at + (chapter_num + 1)}`);

			const options = {
				url: panel_link,
				dest: `../../panels/${title}-chapter-${chapter_to_start_at + (chapter_num + 1)}/${title}-panel-${j + 1}.${extension}`
			}
